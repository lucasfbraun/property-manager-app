...
- **Proprietario**: apenas um cadastro administrativo em `/cadastros` (secao 5) — nao tem login nem portal proprio. Serve para registrar quem e o dono de cada imovel.
...
## 5. Cadastro de proprietarios

Mesma tela `/cadastros`, formulario "Novo proprietario". Campos: nome completo, CPF/CNPJ, e-mail e telefone. Diferente de inquilino e recebedor, o proprietario e **so cadastro** — nao tem senha, login nem portal proprio.

Cada imovel tem exatamente 1 proprietario (campo `owner_id` do imovel). Ao salvar um proprietario, marque na lista de imoveis quais pertencem a ele — a tela avisa com "atual: NomeDoOutroProprietario" se um imovel marcado ja pertence a outro proprietario, porque ao salvar ele e automaticamente transferido para o novo. Todo proprietario precisa ficar vinculado a pelo menos 1 imovel; o formulario nao salva sem isso.

Na lista "Imoveis" (secao 3), cada card agora mostra o nome do proprietario vinculado (ou "Sem proprietario"). Na lista "Proprietarios", cada card mostra os imoveis vinculados a ele. Excluir um proprietario libera os imoveis dele (ficam "Sem proprietario"), sem apagar os imoveis.

Observacao: inquilino e recebedor podem ter o mesmo CPF/CNPJ entre si (por exemplo, a mesma pessoa cadastrada como inquilino em um imovel e recebedor em outro) — nao ha validacao de CPF unico entre os cadastros.

## 6. Cadastro de contratos

Formulario "Novo contrato": escolha inquilino, imovel e recebedor, informe valor do aluguel, dia de vencimento e data de fim. O recebedor fica gravado no contrato e e copiado para cada cobranca gerada (preserva o historico mesmo se o contrato for alterado depois). O formulario tambem tem uma lista de **testemunhas (opcional)**: qualquer recebedor cadastrado pode ser marcado como testemunha do contrato, podendo selecionar mais de um.

Editar um contrato (na tabela "Contratos cadastrados") permite ajustar valor, dia de vencimento, data de fim, status, multa (%), juros ao mes (%), dias de carencia e a lista de testemunhas. Ao salvar um contrato com status "Vence em breve", o inquilino recebe automaticamente um aviso por WhatsApp sobre a proximidade do fim do contrato (uma unica vez).
...
O botao **"Assinaturas"**, na mesma linha, abre um painel de ciencia de assinaturas: um checkbox para o proprietario do imovel (se houver um cadastrado) e um checkbox por testemunha selecionada. Marque cada um conforme for reunindo as assinaturas fisicas do contrato impresso. Veja a secao 10 para como isso libera a assinatura do inquilino.

## 7. Modelos (templates) de contrato
...
## 8. Gerar o contrato de um inquilino
...
**O inquilino sempre assina por ultimo.** Depois que o admin gera o contrato, ele ainda precisa dar ciencia (secao 6, botao "Assinaturas") de que o proprietario do imovel (quando houver um cadastrado) e todas as testemunhas selecionadas ja assinaram o contrato impresso. So depois disso o portal do inquilino libera o link "Ver contrato e assinatura" — antes disso, o inquilino ve uma mensagem de espera no lugar do link (mesmo com o contrato ja gerado). Se o imovel nao tiver proprietario cadastrado, essa etapa e pulada automaticamente; se o contrato nao tiver testemunhas selecionadas, a etapa de testemunhas tambem e considerada satisfeita.

Uma vez liberado, o inquilino ve o texto e as fotos da vistoria no proprio portal, baixa o PDF, assina (manual ou digitalmente) e faz upload do arquivo assinado. Isso entra na fila "Aprovacao de contratos assinados" em `/contratos`, onde o admin pode "Ver documento", "Aprovar" ou "Rejeitar" (com observacao opcional). Status possiveis: nao gerado, aguardando assinatura, em analise, aprovado, rejeitado.

## 11. Ocorrencias reportadas pelo inquilino
...
## 13. Cobranca via Pix (Mercado Pago)
...
## 14. Verificar pagamento (fallback manual)
...
- **Contrato vencendo** (quando o status do contrato vira "Vence em breve", ver secao 6).

Alem do envio automatico, o botao "Enviar lembrete WhatsApp" em `/cadastros` (secao 6) permite disparar manualmente a qualquer momento.

## 19. Seguranca e dados sensiveis
